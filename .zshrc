...
# String of characters which are also part of a word
...
PATH+=":/opt:$HOME/scripts:$HOME/bin:$HOME/.local/bin"
export EDITOR=$(which emacs)

################################################
# Detach some apps

targets=(firefox chromium-browser inkscape gimp eog evince xpdf)
...
if [ -e ~/.zshrc_* ]; then for _rc in ~/.zshrc_*; do
	source $_rc
done; fi
...
#Avoir la complétion lorsque la commande n'est pas dans $PATH :
...
((PROMPT_MAX_SIZE = $COLUMNS - 20))

PROMPT='${PROMPT_LINE}%B%F{green}:%f%b${PR_PWDCOLOR}%~${PR_RESET}${vcs_info_msg_0_}%(80l.
